...
for i in range(len(densities)):
    ...
    # Wykres dla macierzy sąsiedztwa
    plt.plot(vertices[:len(times_matrix[i])], times_matrix[i], marker='o', label='Macierz sąsiedztwa', color='blue')
    
    # Wykres dla listy sąsiedztwa
    plt.plot(vertices[:len(times_list[i])], times_list[i], marker='o', label='Lista sąsiedztwa', color='red')
    
    plt.title(f'Wykres czasu wykonania dla gestości {densities[i]}')
